...
async function main() {
  ...
  */
  ...
  for (let i = 0; i < numberOfTokensToIssue; i += 1) {
      ...
      // Check if the token exists by using the ownerOf function
      try {
          const tokenOwner = await LandToken.ownerOf(tokenId);
          console.log(`Token ${tokenId} is already owned by ${tokenOwner}, skipping...`);
      } catch (error) {
